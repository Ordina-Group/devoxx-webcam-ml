...
async function yolo(modelUrl: string, anchors: number[]): Promise<any> {
    ...
    return async (canvas: HTMLCanvasElement) => {
        const offscreenCanvas: HTMLCanvasElement = new OffscreenCanvas(416, 416) as unknown as HTMLCanvasElement;
        const ctx = offscreenCanvas.getContext('2d');
        ctx.drawImage(canvas, 0, 0, 416, 416);

        const sample = tf.stack([
            tf.div(tf.cast(tf.browser.fromPixels(offscreenCanvas), 'float32'), 255)
        ]);
